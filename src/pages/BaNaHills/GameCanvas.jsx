...
const GameCanvas = ({ onMount }) => {
  ...
  useEffect(() => {
    ...
    class MainScene extends Phaser.Scene {
      ...
      preload() {
        // Tạo texture mặc định cho cable car trong trường hợp không tải được hình ảnh
        this.load.image("cableCar", "cable_car.png");

        // Tạo texture dự phòng trong trường hợp không tải được hình ảnh
        const graphics = this.make.graphics({ x: 0, y: 0, add: false });
        graphics.fillStyle(0x996633);
        graphics.fillRect(0, 0, 80, 60);
        graphics.lineStyle(2, 0x000000);
        graphics.strokeRect(0, 0, 80, 60);
        graphics.generateTexture("cableCarFallback", 80, 60);
      }
      create() {
        ...
        for (let i = 0; i < this.platCount; i++) {
          ...
          const cableY =
            ...
            (platX / this.sys.game.config.width) * 50;
