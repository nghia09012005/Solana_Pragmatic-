...
const LoadingProgress = styled.div`
  ...
  height: 100%;
  background: linear-gradient(90deg, #8B4513, #B22222);
...
const TayNguyenGongsGame = () => {
  ...
  useEffect(() => {
    const loadingMessages = [
      "Đang kết nối với tiếng vọng từ quá khứ...",
      "Đang lắng nghe âm thanh của rừng già...",
      "Đang thu thập mảnh hồn chiêng...",
      "Đang chuẩn bị cho lễ hội...",
      "Đang tìm đường đến buôn làng M'Nông..."
    ];
  ...
  const handleStartGame = () => {
    ...
    if (!audioRef.current) {
      const audio = new Audio(process.env.PUBLIC_URL + '/sound/nhac_nen.mp3');
      audio.loop = true;
      audio.volume = volume;
    ...
    <>
      {isLoading && (
        <LoadingScreen isLoading={isLoading}>
          <LoadingTitle>Hành Trình Cồng Chiêng Tây Nguyên</LoadingTitle>
          <LoadingSubtitle>Lạc vào miền ký ức của đại ngàn</LoadingSubtitle>
          
          <IconsContainer>
            <GoongIcon style={{animationDelay: "0s"}} />
            <GoongIcon style={{animationDelay: "0.5s"}} />
            <GoongIcon style={{animationDelay: "1s"}} />
          </IconsContainer>
      ...
      <GameContainer>
        <VillageScene background={currentBackground}>
          ...
          <ControlContainer show={showMenu}>
            <GameButton onClick={showRandomFact}>Thông tin thú vị</GameButton>
          ...
          <YouTubeMissionBox show={showYouTubeMission}>
            ...
            <VideoContainer>
              <VideoIframe 
                src={`https://www.youtube.com/embed/${getYouTubeVideoId(YouTubeVideoLink)}?rel=0&autoplay=1&mute=0&controls=1`}
